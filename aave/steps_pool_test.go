...
var _ = Describe("Aave Pool write Flow steps", func() {
	It("builds withdraw and repay calls with semantic expectations", func() {
		...
		Expect(steps[0].Expectations[0].ExpectationName()).To(Equal("aave.Withdraw"))
		Expect(steps[1].Expectations[0].ExpectationName()).To(Equal("aave.Repay"))
	})
